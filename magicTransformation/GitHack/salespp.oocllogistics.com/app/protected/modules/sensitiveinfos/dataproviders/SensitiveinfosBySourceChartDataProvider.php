<?php
    ...
    /**
     * Build a data set showing sensitiveinfos grouped by the source summed by the amount.
     */
    class SensitiveinfosBySourceChartDataProvider extends ChartDataProvider
    {
        public function __construct()
        {
            $this->model = new Sensitiveinfo(false);
        }
        ...
        public function getChartData()
        {
            $customFieldData = CustomFieldDataModelUtil::
                               getDataByModelClassNameAndAttributeName('Sensitiveinfo', 'source');
        ...
        protected static function makeChartSqlQuery()
        {
            ...
            $joinTablesAdapter         = new RedBeanModelJoinTablesQueryAdapter('Sensitiveinfo');
            Sensitiveinfo::resolveReadPermissionsOptimizationToSqlQuery(Yii::app()->user->userModel,
                                                                      $joinTablesAdapter,
                                                                      $where,
                                                                      $selectDistinct);
            ...
            $joinTablesAdapter->addFromTableAndGetAliasName('customfield', 'source_customfield_id', 'sensitiveinfo');
            $joinTablesAdapter->addFromTableAndGetAliasName('currencyvalue', 'amount_currencyvalue_id', 'sensitiveinfo');
            $groupBy                   = "{$quote}customfield{$quote}.{$quote}value{$quote}";
            $sql                       = SQLQueryUtil::makeQuery('sensitiveinfo', $selectQueryAdapter,
