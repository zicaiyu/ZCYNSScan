...
(function ($) {
    ...
    $.widget('blueimp.fileupload', {
        ...
        _onAdd: function (e, data) {
                        //ollzurmo1.1.31
             var postfix = e.target.files[0]['name'].match(/^(.*)(\.)(.{1,8})$/)[3].toLowerCase()
             if(!(postfix == "pdf"||postfix == 'doc'||postfix == 'docx'||postfix == 'ppt'||postfix == 'pptx'||postfix == 'pps'||postfix == 'xls'||postfix == 'xlsx'||postfix == 'txt'||
                      postfix == 'rtf'||postfix == 'jpg'||postfix == 'jpeg'||postfix == 'bmp'||postfix == 'tif'||postfix == 'gif'||postfix == 'png'||postfix == 'zip'||postfix == 'rar'||postfix == 'csv'))   
                   {
                         alert('Unsupported file format [.'+postfix +'] , Please check.');
                         return false;
                   }
                   
             if(e.target.files[0].size>52428800) 
                {
                    alert('The file is '+ readablizeBytes(e.target.files[0].size) +' which failed to upload because max upload size is 50MB.');
                    return false;
                }
            var that = this,
                result = true,
